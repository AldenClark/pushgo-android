  ...
  | ((.notes? == null) or (.notes | type == "string"))
    and (
      ...
      or (
        ...
        and (.notesI18n["en"] | type == "string" and (gsub("\\s+"; "") | length > 0))
        and (.notesI18n["zh-CN"] | type == "string" and (gsub("\\s+"; "") | length > 0))
        and (.notesI18n["zh-TW"] | type == "string" and (gsub("\\s+"; "") | length > 0))
      )
    )
' >/dev/null
